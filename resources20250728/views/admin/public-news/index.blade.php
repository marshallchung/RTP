...
'heading' => '最新消息(民眾版)',
'header_btn' => [
'新增', route('admin.public-news.create')
],
'breadcrumbs' => ['最新消息(民眾版)']
])

@section('title', '最新消息(民眾版)')

@section('inner_content')
<div x-data="{
    loading:false,
    getData(page){
        location.href = '{{ route('admin.public-news.index') }}?page=' + encodeURIComponent(page);
    },
    updateActive(e){
        var This=this;
    ...
    items: {{ json_encode($publicNews->items()) }},
    usedKeyboard: false,
    dropcheck: 0,
    ...
    preDragOrder: {{ json_encode($publicNews->items()) }},
    dragstart(event) {
    ...
    editRoute:'{{ route('admin.public-news.' . $routeName, 99999999) }}',
    updateRoute:'{{ route('admin.public-news.update', 99999999) }}',
    routeName:'{{ $routeName }}',
    ...
    <table class="w-full bg-white border shadow-lg text-mainAdminTextGrayDark" @keydown.window.tab="usedKeyboard = true"
        ...
        <tbody class="text-content text-mainAdminTextGrayDark sortable" data-entityname="public-news">
            <template x-for="(news_item, index) in items">
                <tr :x-ref="index" @dragstart="dragstart($event)"
                    ...
                    <td class="p-2 border-r last:border-r-0" x-text="news_item.sort"></td>
                    <td class="p-2 border-r last:border-r-0">
    ...
    <div class="">{!! $publicNews->render() !!}</div>
