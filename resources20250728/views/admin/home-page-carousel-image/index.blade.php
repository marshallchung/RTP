...
'heading' => '首頁輪播設定',
'header_btn' => [
'新增', route('admin.home-page-carousel-image.create')
],
'breadcrumbs' => ['首頁輪播設定']
])

@section('title', '首頁輪播設定')

@section('inner_content')
<div x-data="{
    loading:false,
    getData(page){
        location.href = '{{ route('admin.home-page-carousel-image.index') }}?page=' + encodeURIComponent(page);
    },
    updateActive(e){
        var This=this;
    ...
    items: {{ json_encode($homePageCarouselImages->items()) }},
    usedKeyboard: false,
    dropcheck: 0,
    ...
    preDragOrder: {{ json_encode($homePageCarouselImages->items()) }},
    dragstart(event) {
    ...
    editRoute:'{{ route('admin.home-page-carousel-image.edit', 99999999) }}',
    updateRoute:'{{ route('admin.home-page-carousel-image.update', 99999999) }}',
    ...
    <table class="w-full bg-white border shadow-lg text-mainAdminTextGrayDark" @keydown.window.tab="usedKeyboard = true"
        ...
        <thead>
            <tr>
                ...
                <th class="max-w-xs p-2 font-normal text-left border-r last:border-r-0">連結網址</th>
                <th class="w-24 p-2 font-normal text-left border-r last:border-r-0">圖檔數量</th>
                <th class="w-40 p-2 font-normal text-left border-r last:border-r-0">建立時間</th>
            </tr>
        </thead>
        <tbody class="text-content text-mainAdminTextGrayDark sortable" data-entityname="home-page-carousel-image">
            <template x-for="(homePageCarouselImage, index) in items">
                <tr :x-ref="index" @dragstart="dragstart($event)"
                    @dragend="$event.target.setAttribute('draggable', false)" @dragover="updateListOrder($event)"
                    draggable="false" x-bind:data-itemId="homePageCarouselImage.id"
                    x-bind:data-position="homePageCarouselImage.position"
                    ...
                    <td class="p-2 text-center border-r sortable-handle last:border-r-0">
                        <button type="button" @mousedown="setParentDraggable(event)" @keyup.stop.prevent
                            ...
                            <span x-text="homePageCarouselImage.position"></span>
                        </button>
                    </td>
                    <td class="p-2 border-r last:border-r-0">
                        <div class="flex items-center justify-center w-full h-full">
                            <button @click="updateActive" type="button"
                                ...
                                :class="{'bg-green-500':homePageCarouselImage.active,'bg-black/30':!homePageCarouselImage.active}"
                                x-bind:data-route="updateRoute.replace('99999999',homePageCarouselImage.id)"
                                x-text="homePageCarouselImage.active?'是':'否'"></button>
                        </div>
                    </td>
                    <td class="p-2 border-r last:border-r-0">
                        <a :href="editRoute.replace('99999999',homePageCarouselImage.id)" class="text-mainBlueDark"
                            x-text="homePageCarouselImage.title"></a>
                    </td>
                    <td class="p-2 border-r last:border-r-0">
                        <a :href="homePageCarouselImage.url" class="break-all text-mainBlueDark"
                            x-text="homePageCarouselImage.url"></a>
                    </td>
                    <td class="p-2 border-r last:border-r-0">
                        <div class="flex flex-col items-center justify-center w-full">
                            <span x-text="homePageCarouselImage.files_count"></span>
                            <template x-if="homePageCarouselImage.files_count<=0">
                                <span class="text-rose-600">（無圖片，無法於前端顯示）</span>
                            </template>
                        </div>
                    </td>
                    <td class="p-2 border-r last:border-r-0"
                        x-text="(new Date(homePageCarouselImage.created_at)).toLocaleString('chinese',{hour12:false})">
                    </td>
                </tr>
            </template>
        </tbody>
    </table>
    <div class="">{!! $homePageCarouselImages->render() !!}</div>
