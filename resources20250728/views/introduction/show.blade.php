...
@section('title', "{$introduction->title} - {$introduction->introductionType->name}")
@section('subtitle', $introduction->introductionType->name)
...
<div class="flex flex-row items-start justify-center w-full pb-8">
    <div class="flex flex-col items-start justify-start w-full max-w-screen-lg px-4 py-3 space-y-4">
        <h1 class="pb-1">{{ $introduction->title }}</h1>
        <a href="{{ route('introduction.index', $introduction->introductionType) }}"
            ...
            <span>
                {{ $introduction->introductionType->name }}
            </span>
        </a>
        <div class="relative flex flex-col w-full mt-1 bg-white border rounded">
            <div class="flex-auto p-5">
                {!! $introduction->content !!}
            </div>
        </div>
        @if($introduction->files->count())
        <div class="relative flex flex-col w-full mt-2 bg-white border rounded">
            ...
            <ul class="flex flex-col pl-0 mb-0">
                @foreach($introduction->files as $file)
