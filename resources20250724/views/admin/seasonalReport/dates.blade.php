    ...
    <table class="w-full bg-white border shadow-lg text-mainAdminTextGrayDark">
        <thead>
            <tr class="border-b bg-mainLight">
                <th class="w-24 p-2 font-normal text-left border-r last:border-r-0">年</th>
        ...
        <tbody class="text-content text-mainAdminTextGrayDark">
            ...
            {!! Form::open(['method' => 'POST', 'route' => ['admin.seasonal-report.date.update', $publicDate->id]]) !!}
            <tr>
                <td class="p-2 align-middle border-r last:border-r-0">
                    {{ $publicDate->year }} 
                    {{
                        $publicDate->date_type == 'seasonal0' ? '期初' :
                        ($publicDate->date_type == 'seasonal1' ? '期中' :
                        ($publicDate->date_type == 'seasonal2' ? '期末' : '')) 
                    }}
                </td>
                ...
                <td class="p-2 border-r last:border-r-0">
                    {!! Form::submit('編輯', ['class' => 'flex items-center justify-center w-20 h-9 text-sm text-white bg-mainCyanDark',
                    'style' => 'margin-top: 0;']) !!}
                </td>
            </tr>
            {!! Form::close() !!}
