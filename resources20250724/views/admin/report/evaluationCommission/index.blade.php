...
'heading' => $title,
'breadcrumbs' => [
[$title, route('admin.reports.evaluationCommission.index')],
'管考作業'
]
])
...
<input type="hidden" value="{{ route('admin.reports.evaluationCommission.index') }}" id="js-current-route">

<div x-data="{
    changeYear(e){
        window.location.href = '{{ route('admin.reports.evaluationCommission.index') }}/' + e.target.value;
    }
    ...
    <div class="flex flex-row items-center justify-end mb-4">
        ...
        <select id="js-year-select" @change="changeYear"
                ...
                <div x-show="selectedTab==={{ $index }}" id="area-tab-{{ $index }}"
                    ...
                    <div x-data="{panelOpen:false}" class="flex flex-col border rounded">
                        ...
                        <div x-show="panelOpen" x-transition id="county-{{ $account->id }}" class="flex-col hidden"
                            ...
                            <a href="{{ route('admin.reports.evaluationCommission.show', [$account->id, $year, 'title' => $title]) }}"
                                class="relative block px-5 py-3 text-left border-t">{{
                                $account->name }}政府</a>
                            @foreach ($account->districts['1'] as $district)
                            <a href="{{ route('admin.reports.evaluationCommission.show', [$district->id, $year, 'title' => $title]) }}"
                                class="relative block px-5 py-3 text-left border-t">
