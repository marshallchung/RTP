...
class StoreReportRequest extends Request
{
    ...
    public function rules()
    {
        ...
        if ($topic && $this->get('topic_id_for_validation') == $topicId) {
            do {
                $validation["files.{$filesCount}"] = 'mimes:kml,kmz,bin,xml';
            } while ($filesCount--);
        } else {
            do {
                $validation["files.{$filesCount}"] = 'mimes:pdf,doc,docx,jpg,jpeg,png,gif,zip,rar,txt,csv,xlsx,odf,mp4,mov,ppt,pptx';
            } while ($filesCount--);
        }
