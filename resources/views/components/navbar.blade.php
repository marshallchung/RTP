<nav x-data={popupMenu:false,showMenu:null,showSubMenu:null}
    ...
    <div class="relative flex flex-col items-center justify-center w-full"
        :class="{'{{ count(request()->segments())>0?'bg-black/70 border-b-0':'bg-gradient-to-b from-white to-gray-200 border-b sm:border-b-0 border-b-gray-300' }} shadow-lg':scrollTop>30,'bg-transparent shadow-none':scrollTop<=30}">
        ...
        <div data-tag="上方logo與menu button"
            ...
            <div class="flex flex-row justify-between w-full px-4 py-3 mx-auto item-center flex-nowrap ">
                <a class="navbar-brand py-[0.3125rem] rounded-full w-56" :class="{'-z-10':popupMenu,'z-0':!popupMenu}"
                    href="{{ url('/') }}">
                    <img src="{{ count(request()->segments())>0?'/image/new_logo_white.svg':'/image/new_logo.svg' }}"
                        alt="{{ config('app.cht_name') }}" class="w-full h-auto">
                </a>
...
<div class="relative w-full h-auto mb-16">
    ...
    <h3 class="absolute text-3xl text-white left-12 bottom-16 sm:text-6xl sm:left-24 sm:bottom-28">@yield('title')</h3>
    <h5 class="absolute text-xl text-white left-12 bottom-8 sm:text-3xl sm:left-24 sm:bottom-16">@yield('subtitle')</h5>
</div>
@endif