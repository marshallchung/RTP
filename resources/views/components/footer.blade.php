<footer class="flex flex-col items-center justify-center w-full bg-mainYellow">
    <div class="flex flex-row flex-wrap items-start justify-around w-full max-w-5xl px-4 py-3 text-[rgb(136,136,140)]">
        @foreach(Menu::get('right')->roots() as $menu)
        @if ($menu->title!=='English' && $menu->title!=='業務人員版'&& $menu->title!=='登入'&& $menu->title!=='登出')
        <div class="py-2">
            <h5 class="pb-2 mr-2 text-xl whitespace-nowrap">
                @if($menu->link && $menu->url() != 'javascript:void(0)')
                <a href="{{ $menu->url() }}" class=" hover:no-underline hover:text-mainBlue">{{ $menu->title }}</a>
                @else
            ...
            <ul class="list-none">
                ...
                @if($item->link)
                <li><a href="{{ $item->url() }}" class=" hover:no-underline hover:text-mainBlue">{{ $item->title }}</a>
                </li>
                @else
                <li>{{ $item->title }}</li>
                @endif
            ...
            <div class="flex flex-col items-center justify-start sm:flex-row sm:justify-between">
                <div class="flex flex-col-reverse items-center justify-center flex-1 sm:justify-around sm:flex-row">
                    ...
                    <div
                        ...
                        <span>
                            更新日期：{{ date('Y-m-d') }}&nbsp;|&nbsp;瀏覽人次：
                            {{ $totalVisitorCounter->count }}
                        </span>
                    </div>
                </div>
            </div>
