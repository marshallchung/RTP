        ...
        <table class="w-full bg-white border shadow-lg text-mainAdminTextGrayDark">
            <thead>
                <tr>
                    ...
                    <th class="p-2 font-normal text-left border-r last:border-r-0">參與時間</th>
            ...
            <tbody>
                @foreach ($user->dpExperiences as $item)
                <tr>
                    {!! Form::open([
                    'method' => 'POST',
                    'route' => ['dp.uploadExperienceFiles', $item->id],
                    'files' => true,
                    'class' => 'flex flex-row flex-wrap text-center',
                    ]) !!}
                    <td class="p-2 border-r last:border-r-0">{{ $item->name }}</td>
                    <td class="p-2 border-r last:border-r-0">{{ $item->date }}</td>
                    <td class="p-2 border-r last:border-r-0">{{ $item->unit }}</td>
                    <td class="p-2 border-r last:border-r-0">{{ $item->document_code }}</td>
