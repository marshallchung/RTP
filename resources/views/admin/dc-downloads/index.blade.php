...
'heading' => '推動韌性社區-相關資料',
'header_btn' => ['新增', '/admin/dcDownload/create'],
'breadcrumbs' => ['相關資料列表']
])

@section('title', '相關資料列表')

@section('inner_content')
<div x-data="{
    loading:false,
    getData(page){
        location.href = '/admin/dcDownload/index?page=' + encodeURIComponent(page);
    },
    updateActive(e){
        ...
        .then((response) => {
            if(response.status===200){
                if(activeButton.innerText==='是'){
                    activeButton.innerText = '否';
                    activeButton.classList.remove('bg-green-500');
                    activeButton.classList.add('bg-black/30');
                }else{
                    activeButton.innerText = '是';
                    activeButton.classList.remove('bg-black/30');
    ...
    updatePosition(fromId,fromPosition,toPosition){
        ...
        var token='{{ csrf_token() }}';
        var data = '_method=PUT&_token=' + token+ '&fromId=' + fromId+ '&fromPosition=' + fromPosition + '&toPosition=' +
        toPosition + '&response_json=1';
        this.loading=true;
        fetch(url,{
            method:'POST',
            body:data,
    ...
    preDragOrder: {{ json_encode($data->items()) }},
    dragstart(event) {
    ...
    resetState() {
        if(this.originalIdBeingDragged !==null && this.originalPositionBeingDragged !==null && this.positionBeingDraggedOver
        !==null){
    ...
    editRoute:'/admin/dcDownload/99999999/edit',
    updateRoute:'/admin/dcDownload/99999999',
    ...
    <table class="w-full bg-white border text-mainAdminTextGrayDark border-mainGray"
        ...
        <tbody class="text-content text-mainAdminTextGrayDark sortable" data-entityname="dc_downloads">
            <template x-for="(item, index) in items">
                <tr :x-ref="index" @dragstart="dragstart($event)"
                    ...
                    <td class="flex items-center justify-center p-2 border-r last:border-r-0">
                    ...
                    <td class="p-2 border-r last:border-r-0" x-text="item.category"></td>
                    <td class="p-2 border-r last:border-r-0">
                        <a :href="editRoute.replace('99999999',item.id)" class="text-mainBlueDark"
                            x-text="item.title"></a>
                    </td>
                    <td class="p-2 border-r last:border-r-0" x-text="item.author.name"></td>
                    <td class="p-2 border-r last:border-r-0"
                        x-text="(new Date(item.updated_at)).toLocaleString('chinese',{hour12:false})">
                    </td>
                </tr>
            </template>
        </tbody>
