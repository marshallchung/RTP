                ...
                <tbody class="text-content text-mainAdminTextGrayDark">
                    ...
                    <tr class="bg-white border-b last:border-b-0">
                        ...
                        <td class="p-2 align-middle border-r last:border-r-0">
                            ...
                            <h5><a data-toggle="collapse" role="button" href="#collapse{{ $item->id }}_{{$eachYear}}"
                                    aria-expanded="false" aria-controls="collapse{{ $item->id }}">[+]</a>
                                {{ $eachYear }} 年度</h5>
                            <div class="collapse" id="collapse{{ $item->id }}_{{$eachYear}}">
                                @foreach($files as $file)
                                <div class="p-0 m-0">
                                    {!! Form::open(['method' => 'delete', 'route' =>
                                    ['admin.resultiii.delete-file-in-submit-page', $file], 'class' =>
                                    'flex flex-row flex-wrap text-center', 'style' => 'display: inline-block',
                                    'onsubmit' => 'return
                                    confirm("確定要刪除嗎？")']) !!}
                                    <button type="submit"
                                        class=" px-4 text-sm text-white rounded cursor-pointer py-1.5 bg-rose-600 hover:bg-rose-500">
                                        刪除
                                    </button>
                                    {!! Form::close() !!}
