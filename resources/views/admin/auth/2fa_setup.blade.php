...
<div class="flex items-center justify-center w-screen h-screen bg-gradient-to-t from-gray-200 to-white">
    <div class="flex flex-col items-center justify-start w-full max-w-[34rem] border rounded-3xl bg-white">
        <div class="flex flex-col w-full p-8 space-y-4 text-center text-gray-500">
            ...
            <div class="">首次登入，請設定 Google 驗證器</div>
            <a href="/雙因子驗證登入方法.pdf" target="_blank" style="background: #000; color: #fff; padding: 9px; border-radius: 10px;">雙因子驗證操作方法</a>
        </div>

        <div class="w-full px-8 text-sm text-gray-600">
            <p class="mb-2 font-medium text-gray-700">1. 請使用手機開啟 Google Authenticator 並掃描以下 QR Code：</p>
            <div class="flex items-center justify-center p-4 border rounded bg-gray-50">
                <img src="{{ $qrCodeUrl }}" alt="QR Code">
            </div>
            <p class="mt-4">或手動輸入此金鑰：</p>
            <div class="p-3 mt-1 font-mono text-center text-blue-700 bg-blue-50 rounded">{{ $secret }}</div>
        </div>

        <div class="relative flex items-start justify-start w-full p-8 text-mainAdminTextGrayDark">
            <form method="POST" action="{{ route('auth.2fa.setup.store') }}" class="flex flex-col items-center justify-start w-full">
                @csrf
                <label class="relative flex flex-col items-start justify-start w-full space-y-2">
                    <span class="text-sm text-gray-500">請輸入 6 位數驗證碼</span>
                    <div class="relative flex flex-row w-full">
                        ...
                        <input type="text" name="one_time_password" placeholder="Google 驗證器上的驗證碼"
                ...
                @if(isset($subuser))
                    <input type="hidden" name="sub_user_id" value="{{ $subuser->id }}">
                @endif
