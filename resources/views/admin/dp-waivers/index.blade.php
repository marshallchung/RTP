...
'heading' => '防災士培訓 > 課程抵免申請',

'breadcrumbs' => ['課程抵免申請']
])

@section('title', '課程抵免申請')
...
<div class="col-sm-12 p-0">
    <div class="flex flex-row flex-wrap">
        ...
        <a id="addWaiver" href="" class="btn btn-lg btn-primary" disabled>申請</a>
        {!! Form::close() !!}
    </div>
        ...
        <table id="tableData" class="table table-striped table-bordered">
            <thead>
                <tr>
                    ...
                    <th class="p-2 font-normal text-left border-r last:border-r-0">所屬縣市</th>
...
<script type="text/javascript">
    function getRows (studentsData) {
            ...
            $.each(studentsData, function(idx, el) {
                var tr = '<tr>'+
                    '<td class="p-2 border-r last:border-r-0">'+el.dp_score.dp_student.TID+'</td>'+
                    '<td class="p-2 border-r last:border-r-0">'+el.dp_score.dp_student.county.name+'</td>'+
        ...
        $('#formControl_name').on('change', function() {
            ...
            $.ajax({
                type: 'get',
                dataType: 'json',
                url: '{{ route("admin.dp-waivers.getWaivers") }}',
                data: {
                    course_id: course_id
                },
                success: function (data) {
                    $('#addWaiver').removeAttr('disabled');
                    $('#addWaiver').attr('href', '{{ route("admin.dp-waivers.create") }}?course_id=' + course_id);
