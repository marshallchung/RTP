...
@section('title', '計畫管考項目狀態通知')

@section('inner_content')
<div class="w-full max-w-4xl p-4">
    <div class="w-full px-4 py-6 bg-white border border-gray-200 rounded-sm">
        <div class="w-full mb-10">
            <div class="w-full text-2xl font-bold text-right">歡迎，{{ $user->name }}</div>
            ...
            <div class="pl-24 text-2xl font-bold">{{ $user->name }}您好：</div>
            <div class="pl-24 text-2xl font-bold">今天是{{ intval(date("Y"))-1911 }}年{{ date("n") }}月{{ date("d")
                }}日，計畫管考項目狀態通知如下：</div>
        </div>
        <div class="mb-10">
...
@elseif ($report_public_dates['reports']['expire_date']<=date("Y-m-d") &&
    ...
    <tr class="border-b last:border-b-0">
        ...
        <td class="p-2 border-r last:border-r-0">
            ...
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('plan',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['plan']['expire_date'] && $plan_data[0]['plan_count']==0) <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white bg-yellow-500 rounded-lg h-7 w-[6rem]">
                即將逾期</div>
                @elseif (array_key_exists('plan',$report_public_dates) && $report_public_dates['plan']['expire_date']
                <=date("Y-m-d") && $plan_data[0]['plan_count']==0) <div
                    ...
                    @if (array_key_exists('plan',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['plan']['expire_soon_date'] &&
    ...
    <tr class="border-b last:border-b-0">
        ...
        <td class="p-2 text-center align-middle border-r last:border-r-0">
            ...
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('presentation',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['presentation']['expire_date'] &&
                ...
                $report_public_dates['presentation']['expire_date']<=date("Y-m-d") &&
                    ...
                    @if (array_key_exists('presentation',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['presentation']['expire_soon_date'] &&
                    date("Y-m-d")<=$report_public_dates['presentation']['expire_date'] &&
    ...
    {{-- <tr class="border-b last:border-b-0">
        <td class="p-2 border-r last:border-r-0">優選範本</td>
        <td class="p-2 border-r last:border-r-0">
            {{
            ($report_public_dates &&
        ...
        <td class="p-2 text-center align-middle border-r last:border-r-0">
            ...
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('sample',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['sample']['expire_date'] &&
                ...
                $report_public_dates['sample']['expire_date']<=date("Y-m-d") &&
                    ...
                    @if (array_key_exists('sample',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['sample']['expire_soon_date'] &&
                    date("Y-m-d")<=$report_public_dates['sample']['expire_date'] &&
                        $one_data['report_count']<=$one_data['topic_count']) <div
                        class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white bg-yellow-500 rounded-lg h-7 w-[6rem]">
    ...
    <tr class="border-b last:border-b-0">
        ...
        <td class="p-2 text-center align-middle border-r last:border-r-0">
            ...
            @if ($seasonal_report_2_data[0]['report_count']>0)
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('seasonal0',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['seasonal0']['expire_date'] &&
                ...
                $report_public_dates['seasonal0']['expire_date']<=date("Y-m-d") &&
                    ...
                    @if (array_key_exists('seasonal0',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['seasonal0']['expire_soon_date'] &&
    ...
    <tr class="border-b last:border-b-0">
        ...
        <td class="p-2 text-center align-middle border-r last:border-r-0">
            ...
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('seasonal1',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['seasonal1']['expire_date'] &&
                ...
                $report_public_dates['seasonal1']['expire_date']<=date("Y-m-d") &&
                    ...
                    @elseif ($user->type != 'county' && count($seasonal_report_2_data)>0)
                    @foreach ($seasonal_report_2_data as $one_data)
                    @if (array_key_exists('seasonal1',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['seasonal1']['expire_soon_date'] &&
    ...
    <tr class="border-b last:border-b-0">
        ...
        <td class="p-2 text-center align-middle border-r last:border-r-0">
            ...
            @if ($seasonal_report_3_data[0]['report_count']>0)
            <div
                class="text-center align-middle mb-1 pt-1 mx-auto text-sm text-white rounded-lg h-7 w-[6rem] bg-lime-500">
                已繳交</div>
            @elseif (array_key_exists('seasonal2',$report_public_dates) &&
            ...
            date("Y-m-d")<=$report_public_dates['seasonal2']['expire_date'] &&
                ...
                $report_public_dates['seasonal2']['expire_date']<=date("Y-m-d") &&
                    ...
                    @if (array_key_exists('seasonal2',$report_public_dates) &&
                    date("Y-m-d")>=$report_public_dates['seasonal2']['expire_soon_date'] &&
