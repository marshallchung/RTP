 ...
 * https://github.com/laravel-dojo/twid
 */
class TaiwanIdentityCardService
{
    /**
    ...
    /**
     * The weights which the every number represents.
     *
     * @var array
     */
    protected array $weights = [8, 7, 6, 5, 4, 3, 2, 1, 1];
     ...
     * @return boolean
     */
    public function check(string $id_number = ''): bool
    {
     ...
     * @return boolean
     */
    private function checkIdNumberFormat(string $id_number): bool
    {
        return preg_match('/(^[A-Z]\d{9})/u', $id_number) === 1;
    }
     ...
     * @throws \Exception
     */
    public function make(string $location = null, int $gender = null): string
    {
        if (is_null($location)) {
            $location = array_rand($this->locations);
        } elseif (!(array_key_exists($location, $this->locations))) {
