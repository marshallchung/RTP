...
<?php $__env->startSection('title', '防災士資料管理'); ?>
...
<div x-data="{
    ...
    getData(page){
        location.href = this.makUrl('<?php echo e(route('admin.dp-students.index')); ?>',page);
    },
    deleteItem(url){
        if(confirm('確定要刪除嗎？')){
            var This=this;
    ...
    items: <?php echo e(json_encode($data->items())); ?>,
    editRoute:'<?php echo e(route('admin.dp-students.edit',99999999)); ?>',
    destroyRoute:'<?php echo e(route('admin.dp-students.destroy',99999999)); ?>',
    updateRoute:'<?php echo e(route('admin.dp-students.update', 99999999)); ?>',
    ...
    <div class="flex flex-row flex-wrap items-center justify-start w-full text-sm" id="search-form">
        <form method="GET" action="<?php echo e(request()->url()); ?>" accept-charset="UTF-8"
            ...
            <label class="flex flex-row items-center justify-start m-1 space-x-2">
                <input type="checkbox" name="checkbox_willingness" value='1' <?php echo e(request('checkbox_willingness')?'checked':''); ?>
            ...
            <input
                ...
                placeholder="證書編號" name="certificate" type="text" value="<?php echo e(request('certificate')); ?>">
            <input
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                placeholder="姓名" name="name" type="text" value="<?php echo e(request('name')); ?>">
            <select
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                name="gender">
                <?php $__currentLoopData = $genderOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gender_id=>$gender): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($gender_id); ?>" <?php echo e(request('gender')===$gender_id ?'selected':''); ?>><?php echo e($gender); ?>
            ...
            <input
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                placeholder="培訓單位" name="unit_first_course" type="text" value="<?php echo e(request('unit_first_course')); ?>">
            <select
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                name="county_id">
                <?php $__currentLoopData = $counties; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $county_id=>$county): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($county_id); ?>" <?php echo e(intval(request('county_id'))===$county_id ?'selected':''); ?>><?php echo e($county); ?>
            ...
            <div class="flex flex-row items-center justify-start">
                <select
                    class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                    name="pass">
                    <?php $__currentLoopData = $passOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pass_id=>$pass): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($pass_id); ?>" <?php echo e(strval(request('pass'))===strval($pass_id) ?'selected':''); ?>><?php echo e($pass); ?>
    ...
    <div>合格：<?php echo e($passCount[1] ?? 0); ?>，不合格：<?php echo e($passCount[0] ?? 0); ?></div>
                        ...
                        <td class="p-2 text-center border-r last:border-r-0">
                            <div class="flex items-center justify-center w-full h-full">
                                ...
                                <span class="flex items-center justify-center w-5 h-5 text-xs text-white rounded"
                                    :class="{'bg-green-500':data_item.active,'bg-black/30':!data_item.active}"
                                    x-bind:data-role="<?php echo e(Auth::user()->origin_role); ?>"
    ...
    <div class="pull-right"><?php echo $data->appends(request()->input())->render(); ?></div>
...
'breadcrumbs' => ['防災士資料管理']
], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/Marshall/Downloads/RTP-main/resources/views/admin/dp-students/index.blade.php ENDPATH**/ ?>