...
<div class="bg-mainTopMenuBG relative w-screen min-h-[46px] right-0 z-[1030] text-white flex flex-row items-stretch justify-start h-[46px]"
    ...
    <div class="w-[184px] h-full flex justify-center items-center transition-all duration-500 absolute left-14 top-0"
        ...
        <a href="<?php echo e(route('admin.dashboard.index')); ?>"
            class="text-lg -ml-4 tracking-[0.2rem] whitespace-nowrap transition-all duration-500"><?php echo e(trans('app.name')); ?></a>
    </div>
    <div x-data="{
        openAdminMenu:false,
        ...
        <button type="button" @click="openAdminMenu=!openAdminMenu"
            ...
            data-toggle="dropdown">歡迎，<?php echo e(Auth::user()->name); ?>！</button>
        <ul x-show="openAdminMenu" x-transition
            ...
            <?php if(Auth::user()->hasPermission('admin-permissions')||Auth::user()->hasPermission('NFA-permissions')||Auth::user()->hasPermission('DEP-permissions')): ?>
            <li><a href="<?php echo e(route('admin.users.reset.index')); ?>">帳號管理</a></li>
            <?php endif; ?>
            <?php if(Auth::user()->can_change_identity || Auth::user()->hasPermission('DEP-permissions')): ?>
            <li><a href="<?php echo e(route('admin.identity.index')); ?>">身分切換</a>
            </li>
            <?php endif; ?>
            <li><a href="<?php echo e(route('admin.users.password.index')); ?>">改密碼</a></li>
            <li><a href="<?php echo e(route('admin.auth.logout')); ?>">登出</a></li>
        </ul>
    </div>
</div>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('admin.layouts.pixeladmin', ['bodyClass' => 'main-menu-animated'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/Marshall/Downloads/RTP-main/resources/views/admin/layouts/dashboard.blade.php ENDPATH**/ ?>