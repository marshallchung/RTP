...
<?php $__env->startSection('title', '師資資料庫管理'); ?>

<?php $__env->startSection('inner_content'); ?>
<div x-data="{
    loading:false,
    expired:'<?php echo e(request('expired')); ?>',
    name:'<?php echo e(request('name')); ?>',
    dp_subject:'<?php echo e(request('dp_subject')); ?>',
    ...
    destroyRoute:'<?php echo e(route('admin.dp-teachers.destroy',99999999)); ?>',
    editRoute:'<?php echo e(route('admin.dp-teachers.' . $routeName, 99999999)); ?>',
    mailRoute:'<?php echo e(route('admin.dp-teachers.send-profile-update-mail', 99999999)); ?>',
    items: <?php echo e($data->makeHidden('updated_at')->toJson(JSON_PRETTY_PRINT)); ?>,
    user:<?php echo e(Auth::user()->toJson(JSON_PRETTY_PRINT)); ?>,
        ...
        <form method="GET" action="<?php echo e(request()->url()); ?>" accept-charset="UTF-8"
            ...
            <label class="flex flex-row items-center space-x-2">
                ...
                <select name="expired"
                    ...
                    <option value="" <?php echo e(request('expired')==='' ?'selected':''); ?>>
                        全部
                    </option>
                    <option value="0" <?php echo e(request('expired')==='0' ?'selected':''); ?>>
                        未逾期
                    </option>
                    <option value="1" <?php echo e(request('expired')==='1' ?'selected':''); ?>>
                        逾期
                    </option>
                </select>
            </label>
            ...
            <select x-model="dp_subject"
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                name="dp_subject">
                <?php $__currentLoopData = $dpSubjects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $subject_id=>$dp_subject): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($subject_id); ?>" <?php echo e(strval($subject_id)===request('dp_subject')?'selected':''); ?>>
                    <?php echo e($dp_subject); ?>
            ...
            <select x-model="location"
                class="h-10 px-4 m-1 text-sm border-gray-300 rounded-md shadow-sm w-28 focus:border-cyan-300 focus:ring focus:ring-cyan-200 focus:ring-opacity-50 placeholder:text-sm placeholder:text-mainTextGray"
                name="location">
                <?php $__currentLoopData = $counties; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $county_id=>$county_name): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($county_id); ?>" <?php echo e(strval($county_id)===request('location')?'selected':''); ?>><?php echo e($county_name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
    ...
    <table class="w-full bg-white border shadow-lg text-mainAdminTextGrayDark">
        <thead>
            <tr class="border-b bg-mainLight">
                <?php if(Auth::user()->isAbleTo('DP-teachers-create')): ?>
...
'breadcrumbs' => ['師資資料庫管理']
], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/Marshall/Downloads/RTP-main/resources/views/admin/dp-teachers/index.blade.php ENDPATH**/ ?>